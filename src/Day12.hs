...
mkGraph = foldl (\m (a, b) -> M.insertWith (++) b [a] $ M.insertWith (++) a [b] m) M.empty
...
dfs x res vs m
    | x == "end" = (x:vs):res
    | isLowerS x = concatMap (\x' -> dfs x' res (x:vs) m) (filter (`notElem` vs) (m M.! x))
    | otherwise = concatMap (\x' -> dfs x' res vs m) (filter (`notElem` vs) (m M.! x))
...
dfs2 x res vs m
    | x == "end" = (x:vs):res
    | isLowerS x = concatMap (\x' -> dfs2 x' res (x:vs) m) (filter (myFilter (x:vs)) (m M.! x))
    | otherwise = concatMap (\x' -> dfs2 x' res vs m) (filter (myFilter vs) (m M.! x))
